...
import { NETWORK, positionChanged, smoothPosition } from "../game/network";
...
export default function IronCrownGame({playerName,apiUrl,token,onLogout}:{playerName:string;apiUrl:string;token:string;onLogout:()=>void}){
  ...
  const bossActiveRef=useRef(false),bossIntroRef=useRef(false),nearActionRef=useRef<NearAction>(null),serverBossRef=useRef<ServerBoss|null>(null),socketRef=useRef<ReturnType<typeof io>|null>(null);
  ...
  useEffect(()=>{
    ...
    const syncBoss=(state:ServerBoss,forceUi=false)=>{if(state.regionId!==regionRef.current.id)return;const previous=serverBossRef.current,visual=bossVisualRef.current,hardSync=!visual.initialized||visual.bossId!==state.bossId||previous?.alive!==state.alive||Math.hypot(state.x-visual.renderX,state.y-visual.renderY)>700;serverBossRef.current=state;visual.bossId=state.bossId;visual.targetX=state.x;visual.targetY=state.y;if(hardSync){visual.renderX=state.x;visual.renderY=state.y;visual.initialized=true}const boss=mobsRef.current.find(m=>MONSTERS[m.type].kind==="boss");if(boss&&boss.type===state.bossId){boss.hp=state.hp;boss.phase=state.phase;boss.alive=state.alive;boss.state=state.patternId?"windup":state.engaged?"chase":"idle";boss.aggro=state.engaged}bossActiveRef.current=state.engaged&&state.alive;const uiNow=performance.now();if(forceUi||uiNow-bossUiAtRef.current>180){bossUiAtRef.current=uiNow;if(state.engaged&&state.alive)setBossHp({hp:state.hp,max:state.maxHp,name:MONSTERS[state.bossId].name});else setBossHp(null)}};
    ...
    socket.on("boss:pattern",({boss,effects}:{boss:ServerBoss;effects:BossEffect[]})=>{syncBoss(boss,true);const teleport=effects.find(effect=>effect.move==="teleport");if(teleport)bossVisualRef.current.snapAfter=teleport.resolveAt;bossEffectsRef.current=[...bossEffectsRef.current.filter(effect=>effect.expiresAt>Date.now()-350),...effects].slice(-90);setNotice(`${MONSTERS[boss.bossId].name} · ${boss.patternName??"전투 태세"}`);sound(effects[0]?.cue??"boss")});
    ...
    socket.on("boss:defeated",(state:ServerBoss)=>{syncBoss(state,true);bossEffectsRef.current.length=0;setNotice(`${MONSTERS[state.bossId].name} 처치! 투기장의 봉인이 풀렸습니다.`);shakeRef.current=24;sound("legendary")});
    ...
    send(true);const timer=setInterval(send,1000/NETWORK.presenceSendHz);return()=>{clearInterval(timer);remotePlayersRef.current.clear();socketRef.current=null;socket.disconnect()}
  },[apiUrl,loaded,token]);
  ...
  const travel=useCallback((id:string)=>{
    ...
    bossActiveRef.current=false;bossIntroRef.current=false;serverBossRef.current=null;bossVisualRef.current={bossId:"",renderX:0,renderY:0,targetX:0,targetY:0,initialized:false,snapAfter:0};remotePlayersRef.current.clear();bossEffectsRef.current.length=0;setBossHp(null);projectilesRef.current.length=0;particlesRef.current.length=0;mobsRef.current=makeMobs(target);regionRef.current=target;setRegionId(target.id);setCodexRegion(target.id);
  ...
  const draw=useCallback((ctx:CanvasRenderingContext2D,width:number,height:number,now:number)=>{
    ...
    const visible=(x:number,y:number,margin=90)=>x>cam.x-margin&&x<cam.x+width+margin&&y>cam.y-margin&&y<cam.y+height+margin;
    drawWorld(ctx,currentRegion,cam,width,height,now);drawFacilities(ctx,currentRegion);obstacles.circles.forEach((o,i)=>drawObstacle(ctx,o,currentRegion,i));obstacles.rects.filter(r=>!["building","arena"].includes(r.kind)).forEach(r=>drawRectObstacle(ctx,r,currentRegion));drawArena(ctx,currentRegion);drawBossEffects(ctx,bossEffectsRef.current,Date.now(),cam,width,height);
    const wallNow=Date.now();remotePlayersRef.current.forEach(o=>{if(wallNow-o.lastSnapshot<1500&&o.renderX>cam.x-100&&o.renderX<cam.x+width+100&&o.renderY>cam.y-100&&o.renderY<cam.y+height+100)drawPlayer(ctx,o.renderX,o.renderY,o.facing,o.name,true,undefined,undefined,undefined,o.hp/Math.max(1,o.maxHp),now)});
  ...
  },[currentRegion,obstacles,playerName]);
  ...
  useEffect(()=>{
    ...
    const tick=(now:number)=>{
      ...
      if(inside&&boss?.alive&&!bossActiveRef.current){bossActiveRef.current=true;boss.aggro=true;boss.state="chase";socketRef.current?.emit("boss:engage",{regionId:regionRef.current.id});setBossHp({hp:boss.hp,max:MONSTERS[boss.type].hp,name:MONSTERS[boss.type].name});setNotice(`봉인이 닫혔습니다 · ${MONSTERS[boss.type].name}의 패턴을 공략하세요`);if(!bossIntroRef.current){bossIntroRef.current=true;sound("boss");shakeRef.current=23}}
...
function drawBossEffects(ctx:CanvasRenderingContext2D,effects:BossEffect[],now:number,cam:Vec,width:number,height:number){
  const palettes:Record<string,{main:string;fill:string;accent:string}>={
  ...
  for(const effect of effects){const reach=Math.max(effect.radius,effect.range,effect.width,120);if(now<effect.warnAt||now>effect.expiresAt+350||effect.x+reach<cam.x-80||effect.x-reach>cam.x+width+80||effect.y+reach<cam.y-80||effect.y-reach>cam.y+height+80)continue;const p=palettes[effect.element]??palettes.iron,warning=now<effect.resolveAt,progress=clamp((now-effect.warnAt)/Math.max(1,effect.resolveAt-effect.warnAt),0,1),impact=clamp(1-(now-effect.resolveAt)/350,0,1);ctx.save();ctx.translate(effect.x,effect.y);ctx.rotate(effect.shape==="line"||effect.shape==="cone"?effect.angle:0);ctx.strokeStyle=p.main;ctx.fillStyle=p.fill;ctx.shadowColor=p.main;ctx.shadowBlur=warning?10+progress*18:22*impact;ctx.lineWidth=warning?3+progress*3:7;ctx.globalAlpha=warning?.52+progress*.38:Math.max(.18,impact);ctx.setLineDash(warning?[14,10]:[]);
    ctx.beginPath();
