...
export default function IronCrownGame({playerName,apiUrl,token,onLogout}:{playerName:string;apiUrl:string;token:string;onLogout:(reason?:string)=>void}){
  ...
  const resetCharacter=useCallback(()=>{if(playerRef.current.deadUntil>performance.now())return;if(confirm("캐릭터를 현재 지역 입구에서 다시 시작할까요?"))hurtPlayer(Number.MAX_SAFE_INTEGER)},[hurtPlayer]);
