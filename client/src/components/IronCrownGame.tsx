...
import { BOSS_DEFINITIONS, type BossShape } from "../game/bosses";
import { NETWORK, cameraZoomFor, positionChanged, smoothPosition } from "../game/network";
...
type ServerBoss={regionId:string;bossId:string;x:number;y:number;hp:number;maxHp:number;phase:number;alive:boolean;engaged:boolean;respawnAt:number;targetId:string|null;patternId:string|null;patternName:string|null;vulnerableUntil:number;armor:number;armorMax:number;serverNow:number};
type BossEffect={id:string;bossId:string;patternId:string;patternName:string;name:string;element:string;cue:string;shape:BossShape;x:number;y:number;sourceX:number;sourceY:number;angle:number;radius:number;innerRadius:number;range:number;width:number;arc:number;damage:number;warnAt:number;resolveAt:number;expiresAt:number;repeat:number;move:string;resolved:boolean;nextHitAt:number;hitCycle:number};
type BossTimeline={attackStartAt:number;warnAt:number;resolveAt:number;bossStartX:number;bossStartY:number;targetX:number;targetY:number;direction:number;move:string;serverNow:number};
type RosterPlayer={id:string;name:string;maxHp:number;weapon:string;level:number;region?:string};
type RemotePlayer=RosterPlayer&{renderX:number;renderY:number;targetX:number;targetY:number;hp:number;facing:number;lastSnapshot:number};
type WorldSnapshot={t:number;p:Array<readonly [string,number,number,number]>};
type ServerReward={gold:number;xp:number;levels:number;drops:Equipment[]};
type Panel=null|"inventory"|"forge"|"shop"|"world"|"bestiary"|"help"|"online";
type NearAction="shop"|"forge"|"gate"|null;
type ConfirmModalState={kind:"reset"}|{kind:"delete";item:Equipment}|null;
...
const titleForLevel=(level:number)=>level>=64?"왕좌의 도전자":level>=52?"화산 정복자":level>=41?"설원의 생존자":level>=31?"협곡의 전사":level>=22?"늪지 추적자":level>=14?"광산 탐험가":level>=7?"숲의 사냥꾼":"초원의 모험가";
function compactInPlace<T>(items:T[],keep:(item:T)=>boolean,max=Infinity){let write=0;for(let read=0;read<items.length;read++)if(keep(items[read]))items[write++]=items[read];items.length=write;if(items.length>max)items.splice(0,items.length-max)}
...
export default function IronCrownGame({playerName,apiUrl,token,onLogout}:{playerName:string;apiUrl:string;token:string;onLogout:(reason?:string)=>void}){
  ...
  const mobsRef=useRef<Mob[]>(makeMobs(REGIONS[0])),particlesRef=useRef<Particle[]>([]),textsRef=useRef<FloatText[]>([]),slashesRef=useRef<Slash[]>([]),projectilesRef=useRef<Projectile[]>([]),bossEffectsRef=useRef<BossEffect[]>([]);
  const keysRef=useRef(new Set<string>()),pointerRef=useRef({x:900,y:450,down:false}),cameraRef=useRef({x:0,y:0}),cameraZoomRef=useRef(1),joystickRef=useRef({x:0,y:0,active:false});
  ...
  const bossActiveRef=useRef(false),localArenaLockedRef=useRef(false),engagePendingAtRef=useRef(0),bossIntroRef=useRef(false),nearActionRef=useRef<NearAction>(null),serverBossRef=useRef<ServerBoss|null>(null),socketRef=useRef<ReturnType<typeof io>|null>(null);
  const remotePlayersRef=useRef(new Map<string,RemotePlayer>()),bossVisualRef=useRef({bossId:"",renderX:0,renderY:0,targetX:0,targetY:0,initialized:false,snapAfter:0}),bossUiAtRef=useRef(0);
  const serverClockOffsetRef=useRef(0),serverClockReadyRef=useRef(false),bossMotionRef=useRef({active:false,move:"none",fromX:0,fromY:0,toX:0,toY:0,warnAt:0,resolveAt:0}),bossSoundTimersRef=useRef<ReturnType<typeof setTimeout>[]>([]);
  const combatCallbacksRef=useRef<{hurtPlayer:(damage:number,source?:Mob)=>void;burst:(x:number,y:number,color:string,count:number,power?:number,ring?:boolean)=>void}>({hurtPlayer:()=>{},burst:()=>{}});
  ...
  const [online,setOnline]=useState(1),[roster,setRoster]=useState<RosterPlayer[]>([]),[enhanceResult,setEnhanceResult]=useState(""),[guardMessage,setGuardMessage]=useState("");
  ...
  const [confirmModal,setConfirmModal]=useState<ConfirmModalState>(null),[confirmBusy,setConfirmBusy]=useState(false),confirmModalRef=useRef<ConfirmModalState>(null),confirmBusyRef=useRef(false);
  ...
  useEffect(()=>{confirmModalRef.current=confirmModal},[confirmModal]);useEffect(()=>{confirmBusyRef.current=confirmBusy},[confirmBusy]);
  ...
  useEffect(()=>{
    ...
    apiFetch("/api/save").then(async r=>{if(r.status===401){const data=await r.json().catch(()=>({}));onLogout(data.error??"다른 기기에서 로그인되어 현재 접속이 종료되었습니다.");throw new Error("session replaced")}if(!r.ok)throw new Error("save unavailable");return r.json()}).then(data=>{
  ...
  },[addToast,apiFetch,onLogout]);
  useEffect(()=>{if(!loaded)return;if(persistTimer.current)clearTimeout(persistTimer.current);persistTimer.current=setTimeout(()=>{const payload={...saveRef.current,hp:Math.round(playerRef.current.hp),updatedAt:Date.now()};apiFetch("/api/save",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)}).then(async response=>{if(response.status===401){const data=await response.json().catch(()=>({}));onLogout(data.error??"다른 기기에서 로그인되어 현재 접속이 종료되었습니다.");return}if(!response.ok)console.error("IRON CROWN save failed",response.status,await response.text())}).catch(error=>console.error("IRON CROWN save unavailable",error))},700);return()=>{if(persistTimer.current)clearTimeout(persistTimer.current)}},[save,loaded,apiFetch,onLogout]);
  useEffect(()=>{
    if(!loaded)return;
    const socket=io(apiUrl,{auth:{token},transports:["websocket","polling"]});socketRef.current=socket;
    const syncClock=(serverNow:number)=>{const sample=serverNow-Date.now();if(!serverClockReadyRef.current){serverClockOffsetRef.current=sample;serverClockReadyRef.current=true}else serverClockOffsetRef.current+=clamp(sample-serverClockOffsetRef.current,-120,120)*.12};
    const syncBoss=(state:ServerBoss,forceUi=false)=>{if(state.regionId!==regionRef.current.id)return;syncClock(state.serverNow);const previous=serverBossRef.current,visual=bossVisualRef.current,hardSync=!visual.initialized||visual.bossId!==state.bossId||previous?.alive!==state.alive||Math.hypot(state.x-visual.renderX,state.y-visual.renderY)>700;serverBossRef.current=state;visual.bossId=state.bossId;visual.targetX=state.x;visual.targetY=state.y;if(hardSync){visual.renderX=state.x;visual.renderY=state.y;visual.initialized=true;bossMotionRef.current.active=false}const boss=mobsRef.current.find(m=>MONSTERS[m.type].kind==="boss");if(boss&&boss.type===state.bossId){boss.hp=state.hp;boss.phase=state.phase;boss.alive=state.alive;boss.state=state.patternId?"windup":state.engaged?"chase":"idle";boss.aggro=state.engaged}bossActiveRef.current=state.engaged&&state.alive;const uiNow=performance.now();if(forceUi||uiNow-bossUiAtRef.current>180){bossUiAtRef.current=uiNow;if(state.engaged&&state.alive)setBossHp({hp:state.hp,max:state.maxHp,name:MONSTERS[state.bossId].name});else setBossHp(null)}};
    const lastSent={x:Number.NaN,y:Number.NaN,hp:-1,region:"",maxHp:-1,weapon:"",level:-1,at:0};
    const send=(force=false)=>{const p=playerRef.current,st=statsFromSave(saveRef.current),region=regionRef.current.id,weapon=st.weapon?.name??"맨손",level=saveRef.current.level,hp=Math.round(p.hp),now=performance.now(),staticChanged=region!==lastSent.region||st.maxHp!==lastSent.maxHp||weapon!==lastSent.weapon||level!==lastSent.level,moved=positionChanged(Number.isFinite(lastSent.x)?lastSent:null,p),heartbeat=now-lastSent.at>=NETWORK.presenceHeartbeatMs;if(!force&&!staticChanged&&!moved&&hp===lastSent.hp&&!heartbeat)return;const payload:Record<string,string|number>={x:Math.round(p.x*10)/10,y:Math.round(p.y*10)/10,hp};if(staticChanged||force){payload.region=region;payload.maxHp=st.maxHp;payload.weapon=weapon;payload.level=level}socket.emit("presence",payload);Object.assign(lastSent,{x:p.x,y:p.y,hp,region,maxHp:st.maxHp,weapon,level,at:now})};
    socket.on("world:roster",(players:RosterPlayer[])=>{const active=new Set(players.map(player=>player.id));for(const player of players){if(player.id===socket.id)continue;const current=remotePlayersRef.current.get(player.id);if(current)Object.assign(current,player);else remotePlayersRef.current.set(player.id,{...player,renderX:0,renderY:0,targetX:0,targetY:0,hp:player.maxHp,facing:0,lastSnapshot:0})}for(const id of remotePlayersRef.current.keys())if(!active.has(id))remotePlayersRef.current.delete(id)});
    socket.on("online:roster",(players:RosterPlayer[])=>{setRoster(players);setOnline(players.length)});
    socket.on("world:snapshot",(snapshot:WorldSnapshot)=>{for(const [id,x,y,hp] of snapshot.p){if(id===socket.id)continue;let remote=remotePlayersRef.current.get(id);if(!remote){const created:RemotePlayer={id,name:"모험가",maxHp:Math.max(1,hp),weapon:"맨손",level:1,renderX:x,renderY:y,targetX:x,targetY:y,hp,facing:0,lastSnapshot:snapshot.t};remotePlayersRef.current.set(id,created);continue}const gap=snapshot.t-remote.lastSnapshot,jump=Math.hypot(x-remote.targetX,y-remote.targetY);if(remote.lastSnapshot===0||gap>800||jump>=NETWORK.teleportSnapDistance){remote.renderX=x;remote.renderY=y}const dx=x-remote.targetX,dy=y-remote.targetY;if(Math.hypot(dx,dy)>.5)remote.facing=Math.atan2(dy,dx);remote.targetX=x;remote.targetY=y;remote.hp=hp;remote.lastSnapshot=snapshot.t} });
    socket.on("boss:state",syncBoss);
    socket.on("boss:pattern",({boss,effects,timeline}:{boss:ServerBoss;effects:BossEffect[];timeline:BossTimeline})=>{syncBoss(boss,true);syncClock(timeline.serverNow);const offset=serverClockOffsetRef.current,toLocal=(value:number)=>value-offset,localEffects=effects.map(effect=>({...effect,warnAt:toLocal(effect.warnAt),resolveAt:toLocal(effect.resolveAt),expiresAt:toLocal(effect.expiresAt),nextHitAt:toLocal(effect.nextHitAt)}));const localTimeline={...timeline,warnAt:toLocal(timeline.warnAt),resolveAt:toLocal(timeline.resolveAt),attackStartAt:toLocal(timeline.attackStartAt)};bossMotionRef.current={active:timeline.move!=="none",move:timeline.move,fromX:timeline.bossStartX,fromY:timeline.bossStartY,toX:timeline.targetX,toY:timeline.targetY,warnAt:localTimeline.warnAt,resolveAt:localTimeline.resolveAt};bossEffectsRef.current=[...bossEffectsRef.current.filter(effect=>effect.expiresAt>Date.now()-350),...localEffects].slice(-90);setNotice(`${MONSTERS[boss.bossId].name} · ${boss.patternName??"전투 태세"}`);const delay=Math.max(0,localTimeline.warnAt-Date.now()),timer=setTimeout(()=>requestAnimationFrame(()=>sound(localEffects[0]?.cue??"boss")),delay);bossSoundTimersRef.current.push(timer)});
    socket.on("boss:phase",(state:ServerBoss)=>{syncBoss(state,true);setNotice(`${MONSTERS[state.bossId].name} · ${BOSS_DEFINITIONS[state.bossId]?.phases[state.phase-1]?.name??`${state.phase}페이즈`}`);shakeRef.current=16;sound("boss")});
    socket.on("boss:damage",({bossId,hp,amount}:{bossId:string;hp:number;amount:number})=>{const boss=mobsRef.current.find(m=>m.type===bossId);if(boss){boss.hp=hp;boss.flashUntil=performance.now()+120;textsRef.current.push({x:boss.x+between(-18,18),y:boss.y-46,text:formatNumber(amount),color:"#ffe19a",life:.75,size:19})}});
    socket.on("boss:hit",({bossId,damage}:{bossId:string;damage:number})=>{const boss=mobsRef.current.find(m=>m.type===bossId);combatCallbacksRef.current.hurtPlayer(damage,boss)});
    socket.on("boss:defeated",(state:ServerBoss)=>{syncBoss(state,true);localArenaLockedRef.current=false;bossMotionRef.current.active=false;bossEffectsRef.current.length=0;setNotice(`${MONSTERS[state.bossId].name} 처치! 투기장의 봉인이 풀렸습니다.`);shakeRef.current=24;sound("legendary")});
    socket.on("boss:participation",({regionId,locked}:{regionId:string;locked:boolean})=>{if(regionId!==regionRef.current.id)return;localArenaLockedRef.current=locked;if(!locked)bossEffectsRef.current.length=0});
    socket.on("boss:reset",(state:ServerBoss&{recovered:number})=>{syncBoss(state,true);localArenaLockedRef.current=false;bossMotionRef.current.active=false;bossEffectsRef.current.length=0;setNotice(`${MONSTERS[state.bossId].name} 전투 초기화 · 체력 50% 회복`)});
    socket.on("session:replaced",({message}:{message?:string})=>onLogout(message??"다른 기기에서 로그인되어 현재 접속이 종료되었습니다."));
    socket.on("connect_error",(error)=>{if(/session replaced/i.test(error.message))onLogout("다른 기기에서 로그인되어 현재 접속이 종료되었습니다.")});
    socket.on("boss:reward",({save:next,reward}:{save:SaveData;reward:ServerReward})=>{setSave(next);saveRef.current=next;textsRef.current.push({x:playerRef.current.x,y:playerRef.current.y-55,text:`+${formatNumber(reward.xp)} XP  +${formatNumber(reward.gold)}G`,color:"#ffe7a0",life:1.5,size:17});if(reward.levels){sound("level");addToast(`LEVEL UP!  Lv.${next.level}`,"level")}reward.drops.forEach(item=>{const isLegend=item.rarity==="legendary";sound(isLegend?"legendary":"drop");combatCallbacksRef.current.burst(playerRef.current.x,playerRef.current.y,isLegend?"#ffd653":"#8cff9a",isLegend?60:24,isLegend?340:180,true);if(isLegend){setLegendary(item);setTimeout(()=>setLegendary(null),3700)}addToast(`${isLegend?"전설 ":""}${item.name} 획득!`,isLegend?"legendary":"drop")})});
    socket.on("boss:reward-error",({error}:{error:string})=>addToast(error||"보상을 처리하지 못했습니다.","warn"));
    send(true);const timer=setInterval(send,1000/NETWORK.presenceSendHz);return()=>{clearInterval(timer);bossSoundTimersRef.current.forEach(clearTimeout);bossSoundTimersRef.current=[];remotePlayersRef.current.clear();socketRef.current=null;socket.disconnect()}
  },[apiUrl,loaded,onLogout,sound,token]);
  ...
  const damageMob=useCallback((mob:Mob,damage:number,heavy=false)=>{
    if(!mob.alive)return;const now=performance.now(),bp=MONSTERS[mob.type];
    if(bp.kind==="boss"){
      if(!bossActiveRef.current||!serverBossRef.current?.alive)return;
      mob.flashUntil=now+120;shakeRef.current=Math.max(shakeRef.current,heavy?12:5);hitStopUntilRef.current=Math.max(hitStopUntilRef.current,now+(heavy?52:28));
      burst(mob.x,mob.y,heavy?"#ffd36d":"#eaf8d4",heavy?14:7,heavy?185:105);sound(heavy?"mace":"hit");
      socketRef.current?.emit("boss:damage",{regionId:regionRef.current.id,damage:Math.max(1,Math.round(damage))});return;
    }
    mob.hp-=damage;mob.flashUntil=now+140;mob.aggro=true;mob.aggroUntil=now+14000;if(mob.state==="idle")mob.state="chase";
    ...
    if(mob.hp<=0){mob.hp=0;mob.alive=false;mob.respawnAt=now+bp.respawn*1000;burst(mob.x,mob.y,bp.accent,28,190,true);claimKill(mob.type);addToast(`${bp.name} 처치`,bp.kind)}
  ...
  const hurtPlayer=useCallback((raw:number,source?:Mob)=>{
    ...
    if(p.hp<=0){p.hp=0;p.deadUntil=performance.now()+2400;setDead(true);setNotice(`쓰러졌습니다 · ${regionRef.current.name} 입구로 귀환 중`);setBossHp(null);projectilesRef.current=[];bossEffectsRef.current=[];
      socketRef.current?.emit("boss:leave");localArenaLockedRef.current=false;bossMotionRef.current.active=false;
  ...
  combatCallbacksRef.current={hurtPlayer,burst};
  const requestReset=useCallback(()=>{if(playerRef.current.deadUntil<=performance.now())setConfirmModal({kind:"reset"})},[]);
  ...
  const blocked=useCallback((x:number,y:number,r=18)=>{
    ...
    const wallNow=Date.now();if(bossEffectsRef.current.some(effect=>effect.cue==="ice"&&effect.shape==="circle"&&wallNow>=effect.resolveAt&&wallNow<=effect.expiresAt&&Math.hypot(x-effect.x,y-effect.y)<effect.radius*.68+r))return true;
    if(localArenaLockedRef.current&&x+r>ARENA.x&&x-r<ARENA.x+48&&y+r>ARENA.gateY1&&y-r<ARENA.gateY2)return true;
  ...
  const travel=useCallback((id:string)=>{
    ...
    socketRef.current?.emit("boss:leave");bossActiveRef.current=false;localArenaLockedRef.current=false;bossIntroRef.current=false;serverBossRef.current=null;bossMotionRef.current.active=false;bossVisualRef.current={bossId:"",renderX:0,renderY:0,targetX:0,targetY:0,initialized:false,snapAfter:0};remotePlayersRef.current.clear();bossEffectsRef.current.length=0;setBossHp(null);projectilesRef.current.length=0;particlesRef.current.length=0;mobsRef.current=makeMobs(target);regionRef.current=target;setRegionId(target.id);setCodexRegion(target.id);
  ...
  useEffect(()=>{const down=(e:KeyboardEvent)=>{if(e.code==="Escape"&&confirmModalRef.current){e.preventDefault();if(!confirmBusyRef.current)setConfirmModal(null);return}if(confirmModalRef.current)return;if(["ArrowUp","ArrowDown","ArrowLeft","ArrowRight","Space"].includes(e.code))e.preventDefault();keysRef.current.add(e.code);if(e.code==="Space")attack();if(e.code.startsWith("Shift"))dash();if(e.code==="KeyE")attack(true);if(e.code==="KeyQ")drinkPotion();if(e.code==="KeyF")playerRef.current.blocking=true;if(e.code==="KeyI")setPanel(x=>x==="inventory"?null:"inventory");if(e.code==="KeyR"&&nearActionRef.current)setPanel(nearActionRef.current==="gate"?"world":nearActionRef.current);if(e.code==="Escape")setPanel(null)};
  ...
  const draw=useCallback((ctx:CanvasRenderingContext2D,width:number,height:number,now:number)=>{
    const cam=cameraRef.current,shake=shakeRef.current,p=playerRef.current,zoom=cameraZoomRef.current,viewWidth=width/zoom,viewHeight=height/zoom;
    playerRefGlobal.attackAnim=p.attackAnim;playerPositionGlobal.x=p.x;playerPositionGlobal.y=p.y;
    bossActiveGlobal.value=localArenaLockedRef.current;
    mobsRefGlobal.splice(0,mobsRefGlobal.length,...mobsRef.current);
    ctx.clearRect(0,0,width,height);ctx.save();ctx.translate(between(-shake,shake),between(-shake,shake));ctx.scale(zoom,zoom);ctx.translate(-cam.x,-cam.y);
    const visible=(x:number,y:number,margin=90)=>x>cam.x-margin&&x<cam.x+viewWidth+margin&&y>cam.y-margin&&y<cam.y+viewHeight+margin;
    drawWorld(ctx,currentRegion,cam,viewWidth,viewHeight,now);drawFacilities(ctx,currentRegion);obstacles.circles.forEach((o,i)=>drawObstacle(ctx,o,currentRegion,i));obstacles.rects.filter(r=>!["building","arena"].includes(r.kind)).forEach(r=>drawRectObstacle(ctx,r,currentRegion));drawArena(ctx,currentRegion);drawBossEffects(ctx,bossEffectsRef.current,Date.now(),cam,viewWidth,viewHeight,mobile);
    const wallNow=Date.now();remotePlayersRef.current.forEach(o=>{if(wallNow-o.lastSnapshot<1500&&o.renderX>cam.x-100&&o.renderX<cam.x+viewWidth+100&&o.renderY>cam.y-100&&o.renderY<cam.y+viewHeight+100)drawPlayer(ctx,o.renderX,o.renderY,o.facing,o.name,true,undefined,undefined,undefined,o.hp/Math.max(1,o.maxHp),now)});
    mobsRef.current.forEach(m=>{if(m.alive&&visible(m.x,m.y,220))drawMob(ctx,m,now)});projectilesRef.current.forEach(pr=>{if(!visible(pr.x,pr.y,40))return;ctx.save();ctx.fillStyle=pr.color;ctx.shadowColor=pr.color;ctx.shadowBlur=18;ctx.beginPath();ctx.arc(pr.x,pr.y,pr.radius,0,Math.PI*2);ctx.fill();ctx.restore()});
    slashesRef.current.forEach(s=>{if(!visible(s.x,s.y,s.radius+50))return;const a=s.life/s.max;ctx.save();ctx.globalAlpha=a;ctx.strokeStyle=s.color;ctx.shadowColor=s.color;ctx.shadowBlur=s.wide?32:18;ctx.lineCap="round";ctx.lineWidth=s.wide?28:11;ctx.beginPath();if(s.full)ctx.arc(s.x,s.y,s.radius,0,Math.PI*2);else ctx.arc(s.x,s.y,s.radius,s.angle-(s.wide?1.25:.82),s.angle+(s.wide?1.25:.82));ctx.stroke();ctx.restore()});
    particlesRef.current.forEach(f=>{if(!visible(f.x,f.y,30))return;ctx.save();ctx.globalAlpha=clamp(f.life/f.max,0,1);ctx.fillStyle=f.color;ctx.strokeStyle=f.color;if(f.ring){ctx.lineWidth=2;ctx.beginPath();ctx.arc(f.x,f.y,f.size*clamp(f.life/f.max,.25,1),0,Math.PI*2);ctx.stroke()}else{ctx.beginPath();ctx.arc(f.x,f.y,f.size*clamp(f.life/f.max,.2,1),0,Math.PI*2);ctx.fill()}ctx.restore()});
  ...
  useEffect(()=>{
    ...
    const updateMob=(mob:Mob,dt:number,now:number)=>{
      ...
      if(bp.kind==="boss"){const state=serverBossRef.current,visual=bossVisualRef.current;if(state&&state.bossId===mob.type){const wallNow=Date.now(),motion=bossMotionRef.current;if(motion.active){if(motion.move==="teleport"){visual.renderX=wallNow>=motion.resolveAt?motion.toX:motion.fromX;visual.renderY=wallNow>=motion.resolveAt?motion.toY:motion.fromY}else{const progress=clamp((wallNow-motion.warnAt)/Math.max(1,motion.resolveAt-motion.warnAt),0,1),eased=progress<.5?2*progress*progress:1-Math.pow(-2*progress+2,2)/2;visual.renderX=motion.fromX+(motion.toX-motion.fromX)*eased;visual.renderY=motion.fromY+(motion.toY-motion.fromY)*eased}if(wallNow>=motion.resolveAt){motion.active=false;visual.targetX=motion.toX;visual.targetY=motion.toY}}else{const smoothed=smoothPosition({x:visual.renderX,y:visual.renderY},{x:visual.targetX,y:visual.targetY},NETWORK.bossSmoothRate,dt,650);visual.renderX=smoothed.x;visual.renderY=smoothed.y}mob.x=visual.renderX;mob.y=visual.renderY;mob.hp=state.hp;mob.phase=state.phase;mob.alive=state.alive;mob.aggro=state.engaged;mob.state=state.patternId?"windup":state.engaged?"chase":"idle";if(state.targetId){const target=remotePlayersRef.current.get(state.targetId);const point=target?{x:target.renderX,y:target.renderY}:p;mob.facing=Math.atan2(point.y-mob.y,point.x-mob.x)}}return}
      ...
      if(mob.state==="windup"&&now>=mob.stateUntil){mob.hitDone=false;
        ...
        else if(mob.attackMode==="ranged"){const tx=mob.targetX-mob.x,ty=mob.targetY-mob.y,mag=Math.max(1,Math.hypot(tx,ty)),speed=360;projectilesRef.current.push({x:mob.x,y:mob.y,vx:tx/mag*speed,vy:ty/mag*speed,radius:12,damage:bp.attack,color:bp.accent,life:2.4,source:mob.uid});burst(mob.x,mob.y,bp.accent,15,120);mob.state="recover";mob.stateUntil=now+1100}
        else {mob.state="charge";mob.stateUntil=now+(mob.attackMode==="slam"?500:580)}return}
      if(mob.state==="charge"){
        if(mob.attackMode==="slam"){if(!mob.hitDone){mob.hitDone=true;const radius=155;slashesRef.current.push({x:mob.x,y:mob.y,angle:0,radius,color:bp.accent,life:.5,max:.5,wide:true,full:true});burst(mob.x,mob.y,bp.accent,30,330,true);shakeRef.current=18;if(dist(mob,p)<radius)hurtPlayer(bp.attack*1.2,mob)}}
        else {const ca=Math.atan2(mob.targetY-mob.y,mob.targetX-mob.x),speed=bp.speed*3.8,nx=mob.x+Math.cos(ca)*speed*dt,ny=mob.y+Math.sin(ca)*speed*dt;if(!blocked(nx,ny,bp.radius)){mob.x=nx;mob.y=ny}else mob.stateUntil=now;if(!mob.hitDone&&dist(mob,p)<bp.radius+32){mob.hitDone=true;hurtPlayer(bp.attack*1.08,mob)}}
        if(now>=mob.stateUntil){mob.state="recover";mob.stateUntil=now+980}return}
      if(mob.state==="recover"){if(now<mob.stateUntil)return;mob.state="chase"}
      const ranged=bp.attackStyle==="ranged",preferred=ranged?300:bp.attackStyle==="flank"?135:bp.radius+48;
      if(now>=mob.cooldownUntil&&d<bp.range){mob.attackMode=bp.attackStyle==="ranged"?"ranged":bp.attackStyle==="charge"?"charge":bp.attackStyle==="slam"?"slam":"melee";mob.state="windup";mob.targetX=p.x;mob.targetY=p.y;mob.hitDone=false;mob.stateUntil=now+bp.windup*1000;mob.cooldownUntil=now+bp.cooldown*1000;return}
    ...
    const tick=(now:number)=>{
      ...
      if(inside&&boss?.alive&&!localArenaLockedRef.current&&now>=engagePendingAtRef.current){engagePendingAtRef.current=now+350;boss.aggro=true;boss.state="chase";socketRef.current?.emit("boss:engage",{regionId:regionRef.current.id});setBossHp({hp:boss.hp,max:MONSTERS[boss.type].hp,name:MONSTERS[boss.type].name});if(!bossIntroRef.current){bossIntroRef.current=true;setNotice(`보스 전투에 참가했습니다 · ${MONSTERS[boss.type].name}`);sound("boss");shakeRef.current=23}}
      remotePlayersRef.current.forEach(remote=>{const next=smoothPosition({x:remote.renderX,y:remote.renderY},{x:remote.targetX,y:remote.targetY},NETWORK.remoteSmoothRate,dt);remote.renderX=next.x;remote.renderY=next.y});
      ...
      projectilesRef.current.forEach(pr=>{pr.x+=pr.vx*dt;pr.y+=pr.vy*dt;pr.life-=dt;if(pr.life>0&&dist(pr,p)<pr.radius+19){const source=mobsRef.current.find(m=>m.uid===pr.source);hurtPlayer(pr.damage,source);pr.life=0;burst(pr.x,pr.y,pr.color,16,140)}});
      compactInPlace(projectilesRef.current,pr=>pr.life>0&&!blocked(pr.x,pr.y,pr.radius));
      particlesRef.current.forEach(f=>{f.x+=f.vx*dt;f.y+=f.vy*dt;f.vy+=35*dt;f.life-=dt});compactInPlace(particlesRef.current,f=>f.life>0,700);
      const wallNow=Date.now();compactInPlace(bossEffectsRef.current,effect=>wallNow<=effect.expiresAt+350,90);
      textsRef.current.forEach(t=>{t.y-=35*dt;t.life-=dt});compactInPlace(textsRef.current,t=>t.life>0);slashesRef.current.forEach(s=>s.life-=dt);compactInPlace(slashesRef.current,s=>s.life>0);
      cameraZoomRef.current=cameraZoomFor(rect.width,rect.height,mobile);const viewWidth=rect.width/cameraZoomRef.current,viewHeight=rect.height/cameraZoomRef.current;
      cameraRef.current.x+=(clamp(p.x-viewWidth/2,0,Math.max(0,WORLD.w-viewWidth))-cameraRef.current.x)*Math.min(1,dt*8);cameraRef.current.y+=(clamp(p.y-viewHeight/2,0,Math.max(0,WORLD.h-viewHeight))-cameraRef.current.y)*Math.min(1,dt*8);shakeRef.current=Math.max(0,shakeRef.current-dt*35);
  ...
  const pointerWorld=(e:React.PointerEvent<HTMLCanvasElement>)=>{const r=e.currentTarget.getBoundingClientRect(),zoom=cameraZoomRef.current;return{x:(e.clientX-r.left)/zoom+cameraRef.current.x,y:(e.clientY-r.top)/zoom+cameraRef.current.y}};
  ...
  const requestDelete=(item:Equipment)=>{const current=saveRef.current;if(item.slot==="weapon"&&current.inventory.filter(candidate=>candidate.slot==="weapon").length<=1){addToast("마지막 무기는 삭제할 수 없습니다.","warn");return}if(current.equipped[item.slot]===item.id){addToast("현재 장착 중인 장비는 삭제할 수 없습니다. 먼저 다른 장비를 장착해 주세요.","warn");return}setConfirmModal({kind:"delete",item})};
  const cancelConfirm=()=>{if(!confirmBusyRef.current)setConfirmModal(null)};
  const confirmAction=async()=>{const modal=confirmModalRef.current;if(!modal||confirmBusyRef.current)return;if(modal.kind==="reset"){setConfirmBusy(true);setConfirmModal(null);hurtPlayer(Number.MAX_SAFE_INTEGER);setConfirmBusy(false);return}setConfirmBusy(true);try{const response=await apiFetch("/api/game/delete-item",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({itemId:modal.item.id})}),data=await response.json().catch(()=>({error:"서버 응답을 읽지 못했습니다."}));if(response.status===401){onLogout(data.error);return}if(!response.ok)throw new Error(data.error||"장비를 삭제하지 못했습니다.");setSave(data.save);saveRef.current=data.save;setSelectedItem(null);setConfirmModal(null);addToast(`${modal.item.name}을(를) 삭제했습니다.`,"warn");sound("success")}catch(error){setConfirmModal(null);addToast(error instanceof Error?error.message:"장비를 삭제하지 못했습니다.","warn")}finally{setConfirmBusy(false)}};
  ...
  return <div ref={shellRef} className={`game-shell region-${currentRegion.id}`}>
    ...
    <header className="top-hud"><div className="brand-mark"><span className="crown">♛</span><div><strong>IRON CROWN</strong><small>{currentRegion.english} · COMPLETE WORLD</small></div></div>
      <div className="resource"><span>Lv.{save.level}</span><b>{formatNumber(save.gold)} G</b><button className="online-counter" onClick={()=>setPanel("online")} aria-label={`${online}명 접속 중, 목록 열기`}><i className="online-dot"/>{online}<u>명 접속</u></button></div>
      <button className="icon-btn reset-btn" onClick={requestReset} aria-label="캐릭터 재설정" title="캐릭터 재설정">↻</button><button className="icon-btn" onClick={()=>setMuted(x=>!x)} aria-label="소리 켜기/끄기">{muted?"🔇":"🔊"}</button><button className="icon-btn" onClick={()=>document.fullscreenElement?document.exitFullscreen():shellRef.current?.requestFullscreen()} aria-label="전체화면">⛶</button><button className="icon-btn" onClick={()=>onLogout()} aria-label="로그아웃">↪</button></header>
    ...
    {bossHp&&<div className="boss-bar"><small>REGION BOSS · {BOSS_DEFINITIONS[serverBossRef.current?.bossId??""]?.phases[(serverBossRef.current?.phase??1)-1]?.name}</small><strong>{bossHp.name}</strong><div><span style={{width:`${bossHp.hp/bossHp.max*100}%`}}/></div>{Boolean(serverBossRef.current?.armorMax)&&<div className="boss-armor"><span style={{width:`${(serverBossRef.current?.armor??0)/(serverBossRef.current?.armorMax??1)*100}%`}}/></div>}<em>{serverBossRef.current?.patternName??"빈틈을 노리세요"} · {formatNumber(bossHp.hp)} / {formatNumber(bossHp.max)}</em></div>}
    ...
    {panel&&<div className="panel-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setPanel(null)}}><section className="game-panel"><button className="panel-close" onClick={()=>setPanel(null)}>×</button>
      ...
      {panel==="online"&&<><div className="panel-heading"><small>ONLINE ADVENTURERS</small><h2>현재 접속자</h2><p>IRON CROWN에서 모험 중인 플레이어 {roster.length}명</p></div><div className="online-list">{roster.map(player=><article key={player.id}><span className="online-avatar">♟</span><div><b>{player.name}{player.name===playerName&&<em>나</em>}</b><small>Lv.{player.level} · {titleForLevel(player.level)} · {regionById(player.region??"meadow").name}</small><small>{player.weapon} · HP {formatNumber(player.maxHp)}</small></div><i className="online-dot"/></article>)}</div></>}
      {panel==="help"&&<><div className="panel-heading"><small>FIELD MANUAL</small><h2>전투 도움말</h2></div><div className="help-grid"><article><kbd>WASD</kbd><h3>이동</h3><p>캐릭터는 회전하지 않고 이동 방향으로 좌우 반전됩니다.</p></article><article><kbd>CLICK / SPACE</kbd><h3>무기 공격</h3><p>단검·장검·대검·철퇴의 속도, 범위, 타격음이 모두 다릅니다.</p></article><article><kbd>SHIFT</kbd><h3>대쉬</h3><p>0.2초간 빠르게 이동하지만 무적이 아니며 공격할 수 없습니다.</p></article><article><kbd>F / 우클릭</kbd><h3>방어</h3><p>피해를 줄이는 대신 이동이 느려집니다.</p></article><article><kbd>R</kbd><h3>시설 이용</h3><p>상점, 대장간, 지역 관문 앞에서만 해당 시설이 열립니다.</p></article><article><kbd>Q / E</kbd><h3>포션 / 전설 스킬</h3><p>포션은 즉시 회복, 전설 스킬은 무기마다 다른 직접 공격입니다.</p></article></div><div className="drop-rates"><b>보스 전투 흐름</b><span>공격마다 색·모양·예고 시간이 다릅니다. 첫 공격만 보지 말고 연계와 안전지대를 함께 판단하세요.</span><span>대쉬에는 무적이 없습니다. 강한 연계를 피하면 보스에게 명확한 빈틈이 생깁니다.</span></div></>}
    </section></div>}
    ...
    {confirmModal&&<GameConfirmModal modal={confirmModal} busy={confirmBusy} onCancel={cancelConfirm} onConfirm={confirmAction}/>} 
    {dead&&<div className="death-vignette"/>}
  </div>;
}

function ItemDetail({item,equipped,onEquip,onDelete}:{item:Equipment|null;equipped:boolean;onEquip:(item:Equipment)=>void;onDelete:(item:Equipment)=>void}){
  ...
  return <aside className={`item-detail tier-${item.tier} ${item.rarity}`}><div className="detail-gear"><GearIcon item={item} size={88}/></div><small>{item.rarity==="legendary"?"LEGENDARY":item.rarity==="soul"?"MONSTER SOUL":"NORMAL EQUIPMENT"} · TIER {item.tier}</small><h3>{item.name} <em>+{item.enhance}</em></h3><div className="detail-stat"><span>{label}</span><b>{item.slot==="shield"?`${itemFinalStat(item).toFixed(1)}%p`:item.slot==="soul"?item.soulText:formatNumber(itemFinalStat(item))}</b></div>
    ...
    {item.reflect&&<div className="legend-power"><b>✦ 피해 반사 {item.reflect}%</b><span>실제로 받은 피해 일부를 공격자에게 되돌립니다.</span></div>}{item.moveSpeed&&<div className="legend-power"><b>✦ 이동속도 +{item.moveSpeed}%</b></div>}<p>획득처 · {item.source}</p><button disabled={equipped} onClick={()=>onEquip(item)}>{equipped?"장착 중":"장착하기"}</button><button className="delete-item-btn" onClick={()=>onDelete(item)}>⌫ 아이템 삭제</button></aside>;
}

function GameConfirmModal({modal,busy,onCancel,onConfirm}:{modal:Exclude<ConfirmModalState,null>;busy:boolean;onCancel:()=>void;onConfirm:()=>void}){
  const item=modal.kind==="delete"?modal.item:null,legendary=item?.rarity==="legendary",high=item&&item.enhance>=20,enhanced=item&&item.enhance>=10;
  return <div className="confirm-backdrop" role="presentation"><section className={`game-confirm ${legendary?"legendary-danger":""}`} role="dialog" aria-modal="true" aria-labelledby="confirm-title"><small>{modal.kind==="delete"?"INVENTORY WARNING":"FIELD COMMAND"}</small><h2 id="confirm-title">{modal.kind==="delete"?"아이템 삭제":"캐릭터 재설정"}</h2>{item&&<div className={`confirm-item ${item.rarity}`}><GearIcon item={item} size={62}/><div><small>{item.rarity==="legendary"?"LEGENDARY":item.rarity==="soul"?"MONSTER SOUL":"NORMAL"} · T{item.tier}</small><b>{item.name} <em>+{item.enhance}</em></b></div></div>}<div className="confirm-copy">{modal.kind==="reset"?<><p>현재 캐릭터가 사망하고 이 지역의 입구에서 다시 태어납니다.</p><strong>보스전 참가 중이라면 전멸 판정에 포함됩니다.</strong></>:<><p>이 아이템을 정말 삭제할까요?</p>{legendary&&<strong>전설 장비입니다. 정말 삭제하시겠습니까?</strong>}{high?<strong>높은 강화 단계의 장비입니다. 삭제하면 복구할 수 없습니다.</strong>:enhanced?<strong>강화된 장비입니다.</strong>:null}<em>삭제한 아이템은 복구할 수 없습니다.</em></>}</div><div className="confirm-actions"><button onClick={onCancel} disabled={busy}>취소</button><button className="danger" onClick={onConfirm} disabled={busy}>{busy?"처리 중…":modal.kind==="delete"?"삭제":"재설정"}</button></div></section></div>;
}
...
const BOSS_EFFECT_PALETTES:Record<string,{main:string;fill:string;accent:string}>={
  earth:{main:"#ffd76a",fill:"rgba(182,118,48,.24)",accent:"#fff0a6"},iron:{main:"#e7c58b",fill:"rgba(92,98,108,.3)",accent:"#ffffff"},
  crystal:{main:"#70ebff",fill:"rgba(63,150,205,.25)",accent:"#d7fbff"},poison:{main:"#c4ff63",fill:"rgba(92,191,55,.25)",accent:"#dc76ff"},
  war:{main:"#ff805f",fill:"rgba(165,45,38,.25)",accent:"#ffd09a"},ice:{main:"#9beaff",fill:"rgba(86,173,224,.24)",accent:"#f2fdff"},
  lava:{main:"#ff713c",fill:"rgba(255,73,26,.27)",accent:"#ffd35d"},king:{main:"#e5c4ff",fill:"rgba(113,72,163,.27)",accent:"#fff1a3"},
};
function drawBossEffects(ctx:CanvasRenderingContext2D,effects:BossEffect[],now:number,cam:Vec,width:number,height:number,mobile:boolean){
  const shadowScale=mobile?.74:1;
  for(const effect of effects){const reach=Math.max(effect.radius,effect.range,effect.width,120);if(now<effect.warnAt||now>effect.expiresAt+350||effect.x+reach<cam.x-80||effect.x-reach>cam.x+width+80||effect.y+reach<cam.y-80||effect.y-reach>cam.y+height+80)continue;const p=BOSS_EFFECT_PALETTES[effect.element]??BOSS_EFFECT_PALETTES.iron,warning=now<effect.resolveAt,progress=clamp((now-effect.warnAt)/Math.max(1,effect.resolveAt-effect.warnAt),0,1),impact=clamp(1-(now-effect.resolveAt)/350,0,1);ctx.save();ctx.translate(effect.x,effect.y);ctx.rotate(effect.shape==="line"||effect.shape==="cone"?effect.angle:0);ctx.strokeStyle=p.main;ctx.fillStyle=p.fill;ctx.shadowColor=p.main;ctx.shadowBlur=(warning?10+progress*18:22*impact)*shadowScale;ctx.lineWidth=warning?3+progress*3:7;ctx.globalAlpha=warning?.52+progress*.38:Math.max(.18,impact);ctx.setLineDash(warning?[14,10]:[]);
    ctx.beginPath();
    if(effect.shape==="circle")ctx.arc(0,0,effect.radius,0,Math.PI*2);
    else if(effect.shape==="ring"){ctx.arc(0,0,effect.radius,0,Math.PI*2);ctx.moveTo(effect.innerRadius,0);ctx.arc(0,0,effect.innerRadius,0,Math.PI*2,true)}
    else if(effect.shape==="cone"){ctx.moveTo(0,0);ctx.arc(0,0,effect.range,-effect.arc/2,effect.arc/2);ctx.closePath()}
    else ctx.rect(0,-effect.width/2,effect.range,effect.width);
    ctx.fill("evenodd");ctx.stroke();ctx.setLineDash([]);
    if(effect.element==="crystal"||effect.element==="ice"){ctx.strokeStyle=p.accent;ctx.lineWidth=2;for(let i=0;i<5;i++){const a=i*Math.PI*2/5,r=(effect.radius||Math.min(effect.range,190))*.65;ctx.beginPath();ctx.moveTo(Math.cos(a)*r*.3,Math.sin(a)*r*.3);ctx.lineTo(Math.cos(a)*r,Math.sin(a)*r);ctx.stroke()}}
    if(effect.element==="poison"||effect.element==="lava"){ctx.fillStyle=p.accent;for(let i=0;i<7;i++){const a=i*2.399+effect.x*.001,rr=(effect.radius||Math.min(effect.range,180))*(.22+(i%4)*.16);ctx.globalAlpha=warning?.28+progress*.3:.48;ctx.beginPath();ctx.arc(Math.cos(a)*rr,Math.sin(a)*rr,4+(i%3)*2,0,Math.PI*2);ctx.fill()}}
    if(effect.element==="king"||effect.element==="war"){ctx.strokeStyle=p.accent;ctx.globalAlpha=.55;ctx.lineWidth=2;const r=effect.radius||Math.min(effect.range,210);for(let i=0;i<6;i++){const a=i*Math.PI/3;ctx.beginPath();ctx.moveTo(Math.cos(a)*r*.72,Math.sin(a)*r*.72);ctx.lineTo(Math.cos(a)*r,Math.sin(a)*r);ctx.stroke()}}
    if(!warning&&(effect.cue==="summon"||effect.cue==="clone")){ctx.globalAlpha=.84;ctx.fillStyle=effect.cue==="summon"?"#567c36":"#8d63b8";ctx.strokeStyle=p.accent;ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,-24,effect.cue==="summon"?22:26,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillRect(-20,0,40,48);ctx.beginPath();ctx.moveTo(-16,-40);ctx.lineTo(-4,-22);ctx.lineTo(-28,-24);ctx.closePath();ctx.moveTo(16,-40);ctx.lineTo(4,-22);ctx.lineTo(28,-24);ctx.closePath();ctx.fill();ctx.strokeStyle=effect.cue==="summon"?"#d8bb7a":"#d99cff";ctx.beginPath();ctx.moveTo(16,10);ctx.lineTo(42,-18);ctx.stroke()}
    ctx.restore();if(warning&&progress>.15){ctx.save();ctx.fillStyle=p.accent;ctx.font="900 13px system-ui";ctx.textAlign="center";ctx.strokeStyle="rgba(0,0,0,.72)";ctx.lineWidth=4;const labelY=effect.y-(effect.radius||effect.width/2||55)-16;ctx.strokeText(effect.name,effect.x,labelY);ctx.fillText(effect.name,effect.x,labelY);ctx.restore()}}
}
function drawMob(ctx:CanvasRenderingContext2D,m:Mob,now:number){
  ...
  if(m.state==="windup"&&bp.kind!=="boss"){const pulse=1+Math.sin(now/55)*.08;ctx.strokeStyle="#ff5e55";ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=18;ctx.lineWidth=6;ctx.beginPath();ctx.arc(0,0,(m.attackMode==="slam"?160:bp.radius+32)*pulse,0,Math.PI*2);ctx.stroke();if(["charge","ranged"].includes(m.attackMode)){ctx.setLineDash([18,12]);ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo((m.targetX-m.x)*dir,m.targetY-m.y);ctx.stroke();ctx.setLineDash([])}ctx.shadowBlur=0}
...
function drawMinimap(ctx:CanvasRenderingContext2D,width:number,height:number,r:Region){const short=height<=500,mobile=width<760||short,w=short?100:mobile?112:154,h=short?62:mobile?74:100,x=width-w-(short?8:18),y=short?53:82;ctx.fillStyle="rgba(13,22,20,.84)";ctx.beginPath();ctx.roundRect(x,y,w,h,short?8:13);ctx.fill();ctx.strokeStyle=r.accent;ctx.stroke();ctx.fillStyle=r.ground;ctx.fillRect(x+8,y+8,w-16,h-16);ctx.fillStyle=r.accent;ctx.beginPath();ctx.arc(x+w*.83,y+h*.43,short?6:8,0,Math.PI*2);ctx.fill();mobsRefGlobal.filter(m=>m.alive&&MONSTERS[m.type].kind!=="normal").forEach(m=>{ctx.fillStyle=MONSTERS[m.type].kind==="boss"?"#ff725e":"#65d9ff";ctx.beginPath();ctx.arc(x+8+m.x/WORLD.w*(w-16),y+8+m.y/WORLD.h*(h-16),3,0,Math.PI*2);ctx.fill()});ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(x+8+playerPositionGlobal.x/WORLD.w*(w-16),y+8+playerPositionGlobal.y/WORLD.h*(h-16),4,0,Math.PI*2);ctx.fill()}
