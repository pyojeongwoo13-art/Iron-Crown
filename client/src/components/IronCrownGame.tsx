...
export default function IronCrownGame({playerName,apiUrl,token,onLogout}:{playerName:string;apiUrl:string;token:string;onLogout:()=>void}){
  ...
  useEffect(()=>{
    ...
    const syncBoss=(state:ServerBoss)=>{if(state.regionId!==regionRef.current.id)return;serverBossRef.current=state;const boss=mobsRef.current.find(m=>MONSTERS[m.type].kind==="boss");if(boss&&boss.type===state.bossId){boss.x=state.x;boss.y=state.y;boss.hp=state.hp;boss.phase=state.phase;boss.alive=state.alive;boss.state=state.patternId?"windup":state.engaged?"chase":"idle";boss.aggro=state.engaged}bossActiveRef.current=state.engaged&&state.alive;if(state.engaged&&state.alive)setBossHp({hp:state.hp,max:state.maxHp,name:MONSTERS[state.bossId].name});else if(!state.alive||!state.engaged)setBossHp(null)};
    const send=()=>{const p=playerRef.current,st=statsFromSave(saveRef.current),region=regionRef.current.id;socket.emit("presence",{x:p.x,y:p.y,hp:Math.round(p.hp),maxHp:st.maxHp,weapon:st.weapon?.name??"맨손",region})};
    socket.on("world",(players:OtherPlayer[])=>{setOthers(players.filter(o=>o.id!==socket.id&&o.region===regionRef.current.id));setOnline(players.length)});
    socket.on("boss:state",syncBoss);
    socket.on("boss:pattern",({boss,effects}:{boss:ServerBoss;effects:BossEffect[]})=>{syncBoss(boss);bossEffectsRef.current=[...bossEffectsRef.current.filter(effect=>effect.expiresAt>Date.now()-350),...effects].slice(-90);setNotice(`${MONSTERS[boss.bossId].name} · ${boss.patternName??"전투 태세"}`);sound(effects[0]?.cue??"boss")});
    socket.on("boss:phase",(state:ServerBoss)=>{syncBoss(state);setNotice(`${MONSTERS[state.bossId].name} · ${BOSS_DEFINITIONS[state.bossId]?.phases[state.phase-1]?.name??`${state.phase}페이즈`}`);shakeRef.current=16;sound("boss")});
    ...
    socket.on("boss:defeated",(state:ServerBoss)=>{syncBoss(state);bossEffectsRef.current=[];setNotice(`${MONSTERS[state.bossId].name} 처치! 투기장의 봉인이 풀렸습니다.`);shakeRef.current=24;sound("legendary")});
    ...
    send();const timer=setInterval(send,180);return()=>{clearInterval(timer);socketRef.current=null;socket.disconnect()}
  },[apiUrl,loaded,token]);
  ...
  const travel=useCallback((id:string)=>{
    ...
    bossActiveRef.current=false;bossIntroRef.current=false;serverBossRef.current=null;bossEffectsRef.current=[];setBossHp(null);projectilesRef.current=[];particlesRef.current=[];mobsRef.current=makeMobs(target);regionRef.current=target;setRegionId(target.id);setCodexRegion(target.id);
  ...
  useEffect(()=>{
    ...
    const updateMob=(mob:Mob,dt:number,now:number)=>{
      ...
      if(bp.kind==="boss"){const state=serverBossRef.current;if(state&&state.bossId===mob.type){mob.x=state.x;mob.y=state.y;mob.hp=state.hp;mob.phase=state.phase;mob.alive=state.alive;mob.aggro=state.engaged;mob.state=state.patternId?"windup":state.engaged?"chase":"idle";if(state.targetId){const target=others.find(other=>other.id===state.targetId);const point=target??p;mob.facing=Math.atan2(point.y-mob.y,point.x-mob.x)}}return}
    ...
    const tick=(now:number)=>{
      ...
      bossEffectsRef.current=bossEffectsRef.current.filter(effect=>Date.now()<=effect.expiresAt+350).slice(-90);
...
function drawBossEffects(ctx:CanvasRenderingContext2D,effects:BossEffect[],now:number){
