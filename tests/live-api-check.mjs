...
const kill = await json("/api/game/kill", auth(first.token, { monsterId: "hornBeast" }));
assert.ok(kill.reward.drops.length >= 1); assert.ok(kill.save.gold > before.gold); assert.equal(kill.save.stats.bossKills, 1);
...
const presenceCheck = new Promise((resolve, reject) => {
  ...
  socketA.on("connect", () => socketA.emit("presence", { x: 600, y: 1200, hp: 100, maxHp: 100, weapon: "낡은 장검", region: "meadow" }));
  socketB.on("connect", () => socketB.emit("presence", { x: 650, y: 1200, hp: 100, maxHp: 100, weapon: "낡은 장검", region: "meadow" }));
});
await presenceCheck; socketA.disconnect(); socketB.disconnect();
console.log("LIVE API CHECK PASSED: auth, protected save, boss reward, shop, enhancement, relogin restore, two-player presence");
